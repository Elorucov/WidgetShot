...
namespace WidgetShot
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async void DocLink_Click(object sender, RoutedEventArgs e)
        {
            await Launcher.LaunchUriAsync(new Uri("https://adaptivecards.io/documentation/"));
        }
        ...
        private void RenderWidget(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (_renderer == null)
                {
                    ...
                    _renderer.SetFixedDimensions(Convert.ToUInt32(WidgetPreview.Width), Convert.ToUInt32(WidgetPreview.Height + 72));
                    _renderer.ActionRenderers.Set("Action.OpenUrl", new ButtonActionRenderer());
                    _renderer.ActionRenderers.Set("Action.ShowCard", new ButtonActionRenderer());
                    _renderer.ActionRenderers.Set("Action.Submit", new ButtonActionRenderer());
                    _renderer.ActionRenderers.Set("Action.ToggleVisibility", new ButtonActionRenderer());
                }

                var template = new AdaptiveCardTemplate(TemplateCodeInput.Text);
                var expanded = template.Expand(new EvaluationContext
                {
                    HostJson = "{\"widgetSize\":\"medium\"}",
                    RootJson = DataCodeInput.Text
                });
                ...
                _renderer.HostConfig = AdaptiveHostConfig.FromJsonString(App.Current.RequestedTheme == ApplicationTheme.Dark ? _darkConfig : _lightConfig).HostConfig;
                var card = AdaptiveCard.FromJsonString(expanded);
                RenderedAdaptiveCard renderedAdaptiveCard = _renderer.RenderAdaptiveCard(card.AdaptiveCard);
        ...
        private async void TakeWidgetScreenshot(object sender, RoutedEventArgs e)
        {
            ...
            FileSavePicker fsp = new FileSavePicker()
            {
                ...
                SuggestedFileName = $"WidgetScreenshot_{DateTimeOffset.Now.ToUnixTimeSeconds()}.png",
                SuggestedStartLocation = PickerLocationId.PicturesLibrary
            };
        ...
        private void CodeArea_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            if (e.NewSize.Width >= COLUMN_VIEW_MIN_WIDTH)
            {
                Grid.SetColumnSpan(TemplateCodeInput, 1);
                Grid.SetRowSpan(TemplateCodeInput, 2);
                Grid.SetColumnSpan(DataCodeInput, 1);
                Grid.SetRowSpan(DataCodeInput, 2);
                Grid.SetColumn(DataCodeInput, 1);
                Grid.SetRow(DataCodeInput, 1);
            } else
            {
                Grid.SetColumnSpan(TemplateCodeInput, 2);
                Grid.SetRowSpan(TemplateCodeInput, 1);
                Grid.SetColumnSpan(DataCodeInput, 2);
                Grid.SetRowSpan(DataCodeInput, 1);
                Grid.SetColumn(DataCodeInput, 0);
                Grid.SetRow(DataCodeInput, 2);
            }
        }
    }
}